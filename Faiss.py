...
EMBEDDING_DIM   = 768
TOP_K           = 2   # keep small to avoid context overload
...
def store_summary(summary_text: str):
    ...
    Used as compressed memory archive
    """
    ...
    if summary_text in existing:
        ...
        print("[FAISS] Duplicate summary skipped")
    ...
    meta["entries"][mem_id] = {
        ...
        "timestamp": time.time()   # useful for experiments
    ...
    print(f"[FAISS] Stored compressed memory | total snapshots: {index.ntotal}")
...
def retrieve_relevant(query: str) -> str:
    ...
    """
    Returns small compressed insights

    NOT full context
    used only when summary is weak
    """
    ...
    if result:
        ...
        print(
            ...
            f"[FAISS] Retrieved {len(blocks)} archived insights"
